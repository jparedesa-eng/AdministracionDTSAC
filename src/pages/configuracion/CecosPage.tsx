...
import { Plus, Save, PencilLine, Search, Trash2 } from "lucide-react";
import {
    getCecosState,
    subscribeCecos,
    upsertCeco,
    deleteCeco
} from "../../store/cecosStore";
import type { Ceco } from "../../store/cecosStore";
...
export default function CecosPage() {
    ...
    useEffect(() => {
        const unsubscribe = subscribeCecos(() =>
    ...
    // Estado global
    const { cecos, loading, error } = getCecosState();
    ...
    const totalPages = Math.max(
        1,
        Math.ceil(filteredCecos.length / PAGE_SIZE)
    );
    const start = (page - 1) * PAGE_SIZE;
    const currentRows = filteredCecos.slice(start, start + PAGE_SIZE);
    ...
    const handleDelete = async (id: string, code: string) => {
        if (window.confirm(`¿Estás seguro que deseas eliminar el CECO ${code}?`)) {
            try {
                await deleteCeco(id);
                showToast("success", "CECO eliminado correctamente.");
            } catch (err: any) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const trimmedDesc = descripcion.trim().toUpperCase();

        if (!trimmedCode) {
            setLocalError("Ingresa un código de CECO.");
            return;
        }

        if (trimmedCode.length !== 10) {
            setLocalError("El CECO debe tener exactamente 10 dígitos.");
            return;
        }

        if (!/^\d+$/.test(trimmedCode)) {
            setLocalError("El CECO debe ser numérico.");
            return;
        }
        ...
        try {
            ...
            showToast(
                "success",
                editingId
                    ? "CECO actualizado correctamente."
                    : "CECO registrado correctamente."
            );
            resetForm();
        } catch (err: any) {
            const msg = err?.message ?? "No se pudo guardar el CECO.";
            setLocalError(msg);
            showToast("error", msg);
        } finally {
            setSaving(false);
    ...
    const Pagination = () => {
        // Same pagination logic as other pages
        if (filteredCecos.length === 0) return null;
        ...
        return (
            <nav className="flex items-center justify-between gap-2 py-3 text-sm text-gray-600">
                ...
                <ul className="inline-flex items-center overflow-hidden rounded-lg border bg-white">
                    ...
                    {build().map((it, idx) =>
                        it === "dots" ? (
                            <li key={`dots-${idx}`} className="select-none px-3 text-gray-400">…</li>
                        ) : (
                            <li key={it}>
                                <button
                                    type="button"
    ...
    return (
        <>
            <div className="space-y-5">
                ...
                <div className="px-1 flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-600">
                            Gestiona los Centros de Costo (CECO) válidos.
                        </p>
                    </div>
                ...
                <section className="rounded-2xl border border-gray-200 bg-white p-0 overflow-hidden">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-200 bg-gray-50 px-4 py-3 text-xs text-gray-600">
                        <span>
                            {filteredCecos.length} registro(s) encontrado(s)
                        </span>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-left text-sm">
                            <thead className="bg-gray-50 text-xs uppercase text-gray-500">
                                <tr>
                                    <th className="px-4 py-3">Código CECO</th>
                            ...
                            <tbody className="divide-y divide-gray-100 bg-white">
                                ...
                                {!loading && filteredCecos.length === 0 && (
                                    <tr>
                                        <td colSpan={3} className="px-4 py-6 text-center text-sm text-gray-500">
                                ...
                                {!loading && filteredCecos.length > 0 && currentRows.map((c: Ceco) => (
                                        ...
                                        <td className="px-4 py-3 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                ...
                                                <button
                                                    ...
                                                    className="inline-flex items-center justify-center rounded-lg p-2 bg-rose-50 text-rose-600 hover:bg-rose-100 hover:text-rose-700 transition-colors"
                                                >
            ...
            <Modal
                open={formOpen}
                onClose={resetForm}
                title={editingId ? "Editar CECO" : "Registrar nuevo CECO"}
            >
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="mb-1 block text-xs font-medium text-gray-600">
                            Código CECO (10 Dígitos) <span className="text-red-500">*</span>
                        </label>
                        <input
                            required
                            type="text"
                            ...
                            className="w-full rounded-lg border bg-white px-3 py-2 text-sm outline-none ring-1 ring-transparent focus:ring-gray-300 font-mono"
                        />
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Ej. GERENCIA AGRICOLA"
