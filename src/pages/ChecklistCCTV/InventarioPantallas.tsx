...
export default function InventarioPantallas() {
    ...
    const [, setPantallasVersion] = useState(0);

    useEffect(() => {
        const unsubCentrales = subscribeCentrales(() => setCentralesVersion(prev => prev + 1));
        const unsubPantallas = subscribePantallas(() => setPantallasVersion(prev => prev + 1));
        ...
        <div className="space-y-6 pb-20">
            ...
            <div className="min-h-[400px]">
                <TabPantallas
                    ...
                    onAdd={async (data) => {
                        try {
                            await upsertPantalla(data);
                            setToast({ type: "success", message: "Pantalla creada correctamente." });
                        } catch (error: any) {
                            setToast({ type: "error", message: error.message || "Error al crear pantalla." });
                        }
                    }}
                    onUpdate={async (id, data) => {
                        try {
                            const item = pantallas.find(c => c.id === id);
                            if (!item) return;
                            await upsertPantalla({ ...item, ...data, id });
                            setToast({ type: "success", message: "Pantalla actualizada." });
                        } catch (error: any) {
                            setToast({ type: "error", message: error.message || "Error al actualizar pantalla." });
                        }
                    }}
                />
            </div>
...
function TabPantallas({
    ...
    const filtered = pantallas.filter(c => {
    ...
    useEffect(() => {
        ...
        // Find existing screens for this central
        const existingCodes = pantallas
            .filter(p => p.central_id === formData.central_id)
            .map(p => p.codigo);

        // Format: P-[Central Name]-XXX
        // Example: P-CCTV AQP-001
        let maxNum = 0;

        existingCodes.forEach(code => {
            // We look for the pattern and extract the last number
        ...
        const nextCode = `P-${central.nombre}-${nextNum}`;

        setFormData(prev => ({ ...prev, codigo: nextCode }));
    }, [formData.central_id, centrales, pantallas, modalOpen, editItem]);

    const handleOpen = (item?: Pantalla) => {
        if (item) {
            setEditItem(item);
            setFormData({
                codigo: item.codigo,
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between gap-4 px-1">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        Monitores y pantallas de vigilancia
                    </p>
                </div>
                <button
                    onClick={() => handleOpen()}
                ...
                <div className="flex flex-wrap items-center gap-6">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <select
                            ...
                            className="bg-white border text-sm font-semibold px-4 py-2 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 min-w-[180px]"
                        >
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 text-gray-500 font-bold border-b border-gray-200">
                            <tr>
                                ...
                                <th className="px-3 py-2 uppercase tracking-wider">Pulgadas</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {currentRows.map(c => {
                                ...
                                return (
                                    <tr key={c.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-3 py-1.5">{c.pulgadas || "-"}</td>
                            ...
                            {currentRows.length === 0 && (
                                <tr>
                                    <td colSpan={7} className="px-6 py-8 text-center text-gray-500 text-xs italic">
            ...
            <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? "Editar Pantalla" : "Nueva Pantalla"} size="lg">
                <div className="space-y-4 mt-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input value={formData.pulgadas} onChange={e => setFormData({ ...formData, pulgadas: e.target.value })} className="w-full h-11 rounded-lg border px-3" />
                        </div>
                    </div>
