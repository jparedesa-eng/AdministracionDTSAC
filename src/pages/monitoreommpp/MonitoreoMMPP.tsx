...
export const MonitoreoMMPP: React.FC = () => {
    ...
    const getStatusIcon = (status: string) => {
        switch (status) {
            ...
            case 'LLEGADO': return <CheckCircle2 className="w-4 h-4 text-green-600" />;
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            case 'LLEGADO': return 'bg-green-100 text-green-700 border-green-200';
        ...
        <div className="p-8 min-h-screen bg-gray-50/50">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 mt-2">
                <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-500 font-medium">Seguimiento en tiempo real de unidades de campo</p>
                </div>
                        ...
                        <div key={record.id} className="group bg-white rounded-2xl border border-gray-200 shadow-sm hover:border-[#ff0000]/50 hover:shadow-md transition-all duration-200 overflow-hidden flex flex-col h-full uppercase">
                            ...
                            <div className="p-5 pb-3">
                                <div className="flex items-start justify-between mb-3">
                                    <div className="flex items-center gap-2.5">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <span className={`flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-black tracking-wider border uppercase ${getStatusColor(record.estado)}`}>
                                                    {getStatusIcon(record.estado)}
                                                    {record.estado}
                                                </span>
                                            </div>
                                            <p className="text-slate-400 text-[10px] font-bold mt-0.5 uppercase tracking-tight flex items-center gap-1">
                            ...
                            <div className="mt-auto p-4 bg-slate-50 border-t border-gray-200 space-y-3">
                                ...
                                <button 
                                    ...
                                    disabled={record.estado === 'LLEGADO' || record.estado === 'CANCELADO'}
                                    className="w-full py-2.5 bg-white border border-[#ff0000]/20 text-[#ff0000] font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-[#ff0000] hover:text-white transition-all active:scale-[0.98] disabled:opacity-50 disabled:hover:bg-white disabled:hover:text-[#ff0000]"
                                >
                                    Reportar Ubicación
                                </button>
                            </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-900 border-b border-slate-800">
                                <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest">Placa/Unidad</th>
                                <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest">Estado</th>
                                <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest">Origen/Destino</th>
                                <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest">Materia Prima</th>
                                <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest text-center">T. Recorrido</th>
                                <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest text-right">Ubicación</th>
                                <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest text-center">Acciones</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200">
                            {filteredRecords.map((record) => (
                                <tr key={record.id} className="hover:bg-slate-50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black tracking-widest border uppercase ${getStatusColor(record.estado)}`}>
                                            {getStatusIcon(record.estado)}
                                            {record.estado}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[10px] font-black text-slate-400">{record.peso_neto_bruto.toLocaleString()} KG</span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-red-50 text-[#ff0000] rounded-lg text-xs font-bold border border-red-100">
                                            ...
                                            {record.ubicaciones && record.ubicaciones.length > 0 
                                                ? record.ubicaciones[record.ubicaciones.length - 1].ubicacion 
                                                : (record.estado === 'LLEGADO' ? record.destino : 'ORIGEN')}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex justify-center gap-2">
                                            <button 
                                                onClick={() => handleReportLocation(record)} 
                                                disabled={record.estado === 'LLEGADO'}
